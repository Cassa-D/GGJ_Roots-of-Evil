    ...
    public class PlayerController : MonoBehaviour, IPlayerController {
        ...
        private void GatherInput() {
            ...
            Input = new FrameInput {
                JumpDown = _jumpState == JumpState.Pressed,
                JumpUp = _jumpState == JumpState.Released,
                X = _moveAction.ReadValue<float>(),
                IsRunning = _runAction.ReadValue<float>() > 0
            };
            if (Input.JumpDown) {
        ...
        private void Start()
        {
            _moveAction = playerInput.actions["MoveX"];
        ...
        private void CalculateWalk() {
            if (Input.X != 0)
            {
                var runPower = Input.IsRunning ? _runPower : 1;
                
                // Set horizontal move speed
                _currentHorizontalSpeed += Input.X * _acceleration * runPower * Time.deltaTime;

                // clamped by max frame movement
                _currentHorizontalSpeed = Mathf.Clamp(_currentHorizontalSpeed, -_moveClamp * runPower, _moveClamp * runPower);
